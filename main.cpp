...
int main(int argc, char **argv) {
    ...
    for(int i = 0; i < sCommands.size(); i++) {
		if (sCommands.at(i) == ";;") {
			...
			stackstuff.push(sCommands.at(i).substr(sCommands.at(i).find("(") + 1, sCommands.at(i).find(")") - 1), int8);
		} else if (std::strncmp(input.c_str(), "push int16(", 11) == 0) {
			stackstuff.push(sCommands.at(i).substr(sCommands.at(i).find("(") + 1, sCommands.at(i).find(")") - 1), int16);
		} else if (std::strncmp(sCommands.at(i).c_str(), "push int32(", 11) == 0) {
			stackstuff.push(sCommands.at(i).substr(sCommands.at(i).find("(") + 1, sCommands.at(i).find(")") - 1), int32);
		} else if (std::strncmp(sCommands.at(i).c_str(), "push float(", 11) == 0) {
			stackstuff.push(sCommands.at(i).substr(sCommands.at(i).find("(") + 1, sCommands.at(i).find(")") - 1), Float);
		} else if (std::strncmp(sCommands.at(i).c_str(), "push double(", 12) == 0) {
			stackstuff.push(sCommands.at(i).substr(sCommands.at(i).find("(") + 1, sCommands.at(i).find(")") - 1), Double);
